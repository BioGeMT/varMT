...
def create_tables(dbname: str, user: str, password: str, host: str) -> None:
    ...
    cur.execute("""
            CREATE TABLE variant_frequencies(
                ...
                hom_ref_count INTEGER NOT NULL,
                hom_alt_count INTEGER NOT NULL,
    ...
    cur.execute("""
            CREATE TABLE variant_vep_annotations(
                id SERIAL PRIMARY KEY,
                variant_id INTEGER NOT NULL,
                transcript_id TEXT,
                hgvs_c TEXT,
...
def insert_variant_frequency() -> str:
    ...
    variant_frequency_insert_query = """
                            INSERT INTO variant_frequencies (variant_id, collection_id, alternate_allele_count, 
                                        allele_number, hom_ref_count, hom_alt_count, het_count, missing_count)
...
def insert_variant_vep_annotation() -> str:
    """
    Returns the SQL query string for inserting a variant annotation in the 'variant_vep_annotations' table.
    """
    variant_vep_annotation_insert_query = """
                            INSERT INTO variant_vep_annotations (variant_id, transcript_id, hgvs_c, hgvs_p, consequence, impact)
                            VALUES (%s, %s, %s, %s, %s, %s)
                        """
    return variant_vep_annotation_insert_query
